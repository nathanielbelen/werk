...
export default function Application({ data }) {
  const { company, position, location, subtitle, id, cover_letter, created_at, notes, stage, url, status, resume_number, category } = data;
  ...
  return (
        <AccordionItem borderRadius="lg"
          ...
          textAlign="left"
          overflow="hidden"
          shadow='xs' borderStyle="none" marginBottom="4">
          <AccordionButton p={0} sx={{
            '&:hover': {
              bg: 'none', // Remove or modify the background color change on hover
            },
          }}>
            <Box display="flex" w={"100%"} alignItems="stretch">
              ...
              <Flex justify="space-between" flex={1} my={5} mx={3}>
                ...
                <Flex flexDirection="column" justifyContent="center" gap={1}>
                  ...
                  <Chips stage={stage} cover_letter={cover_letter} resume_number={resume_number} category={category} />
                </Flex>
              </Flex>
